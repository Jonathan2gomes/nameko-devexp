...
@pytest.mark.usefixtures('order_details')
def test_get_order_by_product_id(orders_rpc):
    response = orders_rpc.get_order_by_product_id('the_odyssey')

    print('oloco meu')
    print(response)

    assert response == {'id': 1,
                        'order_details': [{'product_id': 'the_odyssey', 'id': 1, 'quantity': 1, 'price': '99.51'},
                                          {'product_id': 'the_enigma', 'id': 2, 'quantity': 8, 'price': '30.99'}]}


@pytest.mark.usefixtures('order_details')
def test_get_order_by_product_id_inexistent(orders_rpc):
    with pytest.raises(RemoteError) as err:
        orders_rpc.get_order_by_product_id('the_curupira')
    assert err.value.value == 'Product with id the_curupira is not associated with any order'
